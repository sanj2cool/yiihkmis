...
function getCategories($parent_id = null, $level = 0, $categories = []) {
    ...
    $query = (new Query())
        ->select(['id', 'title'])
        ->from('tbl_product_assembly_category')
...
<div class="tbl-product-assembly-category-form">
  <div id="msg">
    ...
    <div class="card-body p-3">
      <div class="row">
        <div class="col-lg-6">
          ...
          <?= $form->field($model, 'status')->dropDownList([1=>"Active",2=>"Inactive"],['class'=>'form-select']) ?>
        </div>
        ...
        <div class="col-lg-3">
          ...
          <?php
            if(!$model->isNewRecord && $model->image_url != ""){
              echo '  <a href="assembly-category/'.$model->image_url.'" target="_blank">
                <img src="assembly-category/'.$model->image_url.'" width="100px" class="mt-2"/>
                </a>';
            }
           ?>
        </div>
        <div class="col-lg-3">
          <label for="">Second Level Image</label>
          <input type="file" name="second_level_img" accept="image/*" class="form-control">

          <?php
            if(!$model->isNewRecord && $model->exploded_image_url != ""){
              echo '  <a href="assembly-category/'.$model->exploded_image_url.'" target="_blank">
                <img src="assembly-category/'.$model->exploded_image_url.'" width="100px" class="mt-2"/>
                </a>';
            }
           ?>
        </div>
