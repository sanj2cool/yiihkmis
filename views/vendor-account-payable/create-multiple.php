...
if($client_id_received != ""){
  //----get location -----
  $get_location = TblVendor::find()->where(['id'=>$client_id_received])->andWhere(['fk_location_id'=>$user_company])->andWhere('status != 0')->one();
  if(isset($get_location) && $get_location->id != ""){
    $fk_preferred_payment_method_id = $get_location->fk_preferred_payment_method_id;
    $clients = TblVendor::find()
    ->where(['status'=>1])
...
<div class="tbl-account-receivable-create">
  ...
  <form method="post" action="<?=Url::to(['vendor-account-payable/create-multiple'])?>" enctype="multipart/form-data" id="form_multiple">
    ...
    <div class="card shadow rounded mt-4">
      ...
      <div class="card-body p-3">
        <div class="row">
          ...
          <div class="col-lg-3 mb-2">
            <label for="">Paid Date<span class="text-danger">*</span></label>
...
function getclientinvoices(){
  ...
  if(error == 0){
    $.post("index.php?r=vendor-account-payable/get-pending-invoice",{client_id:client_id},function(r){
      console.log(r);
