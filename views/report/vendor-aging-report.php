...
<div class="card mb-4">
  <div class="card-header bg-light">
    ...
    <form name="frm_reservation" id="" action="<?= Yii::$app->urlManager->createUrl(['report/vendor-aging-report']) ?>" method="post" autocomplete="off">
        ...
        <div class="col-md-3">
            ...
            <select class="form-control" id="customer" name="customer" required="required" data-choices='data-choices' data-options='{"removeItemButton":true,"placeholder":true}'>
                ...
                <?php
                    $allclients = TblVendor::find()->where('status != 0')->andWhere(['fk_location_id'=>$user_company])->orderBy(['company_name'=>SORT_ASC])->all();
          ...
          <tbody>
            ...
            $subquery = (new Query())
                ...
                $query = (new Query())
                    ...
                    ->leftJoin(['sr' => $subquery], 'i.id = sr.fk_vendor_invoice_id')
                    ->where('i.status != 0')
                    ->groupBy([
                        'c.company_name',
                        'c.id',
            ...
            if (!empty($user_company)) {
                if($user_company != 0){
                  $query->andWhere(['i.fk_location_id' => $user_company]);
                }
            }
