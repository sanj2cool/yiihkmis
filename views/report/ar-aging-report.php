...
<div class="card">
  <div class="card-header bg-light">
    ...
    <form name="frm_reservation" id="" action="<?= Yii::$app->urlManager->createUrl(['report/ar-aging-report']) ?>" method="post" autocomplete="off">
      ...
      <table id="example" class="display table table-hover table-bordered border border-1" data-page-length="20">
          ...
          <tbody>
            ...
            // Subquery to calculate the total amount received per invoice
                ...
                if (!empty($from_date) && !empty($to_date)) {
                  $subquery->andWhere(['between', 'ir.ar_date', $from_date, $to_date]);
                }
                $query = (new Query())
                    ->select([
                        'c.company_name',
                        'c.id as cust_id',
                        'ROUND(SUM(i.total_amount), 2) AS total_invoice_amount',
                        'ROUND(SUM(COALESCE(ir.total_amount_received, 0)), 2) AS total_amount_received',
                        'ROUND(SUM(i.total_amount) - SUM(COALESCE(ir.total_amount_received, 0)), 2) AS total_outstanding_amount',
                        'ROUND(SUM(CASE WHEN DATEDIFF(CURDATE(), i.due_date) BETWEEN 0 AND 15 THEN (i.total_amount - COALESCE(ir.total_amount_received, 0)) ELSE 0 END), 2) AS current',
                        'ROUND(SUM(CASE WHEN DATEDIFF(CURDATE(), i.due_date) BETWEEN 16 AND 30 THEN (i.total_amount - COALESCE(ir.total_amount_received, 0)) ELSE 0 END), 2) AS days_16_30',
                        'ROUND(SUM(CASE WHEN DATEDIFF(CURDATE(), i.due_date) BETWEEN 31 AND 60 THEN (i.total_amount - COALESCE(ir.total_amount_received, 0)) ELSE 0 END), 2) AS days_31_60',
                        'ROUND(SUM(CASE WHEN DATEDIFF(CURDATE(), i.due_date) BETWEEN 61 AND 90 THEN (i.total_amount - COALESCE(ir.total_amount_received, 0)) ELSE 0 END), 2) AS days_61_90',
                        'ROUND(SUM(CASE WHEN DATEDIFF(CURDATE(), i.due_date) > 90 THEN (i.total_amount - COALESCE(ir.total_amount_received, 0)) ELSE 0 END), 2) AS over_90_days',
                    ])
                    ->from('tbl_invoice i')
            ...
            foreach($results as $r){
              ...
              echo '<tr>
                <td>'.$r['company_name'].' |
                <a target="_blank" href="'.Url::to(['report/ar-aging-report-details','customer_id'=>$r['cust_id'],'start_date'=>$from_date,'end_date'=>$to_date]).'"><i class="fas fa-external-link-alt" data-bs-toggle="tooltip" data-bs-placement="right" title="Open in New Tab"></i></a></td>
...
//         SUM(COALESCE(ir.amount_received, 0)) AS total_amount_received,
//         SUM(i.total_amount) - SUM(COALESCE(ir.amount_received, 0)) AS total_outstanding,
//         SUM(CASE
//                 WHEN DATEDIFF(CURDATE(), i.due_date) BETWEEN 0 AND 30 THEN i.total_amount - COALESCE(ir.amount_received, 0)
//                 ELSE 0
//             END) AS days_0_30,
//         SUM(CASE
...
//             END) AS days_over_90
//             from tbl_invoice i
// left join tbl_client c on i.fk_client_id = c.id
// left join tbl_account_receivable ir on i.id = ir.fk_invoice_id
// group by
//         c.name,
//         c.id
// HAVING
