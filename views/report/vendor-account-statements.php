<?php
  $this->title = "Vendor Account Statements";
...
<div class="card mb-4">
	...
	<div class="card-body border-top">
        <form name="frm_reservation" id="" action="<?= Yii::$app->urlManager->createUrl(['report/vendor-account-statements']) ?>" method="post" autocomplete="off">
                        ...
                        if(isset($from_date) && isset($to_date) && isset($customer)){
                          $bill_company = $user_company;
                              $getinvoices = \app\models\TblVendorInvoice::find()
                                              ...
                                              ->andWhere(['fk_vendor_id'=>$customer])
                                              ->andWhere(['fk_location_id'=>$bill_company])
                            ...
                            if(isset($getinvoices) && count($getinvoices) > 0){
                                foreach($getinvoices as $gi){
                                    $invoice_arr[] = ['date'=>date('Y-m-d',strtotime($gi->crt_time)),"type"=>"Invoice","number"=>$gi->vendor_invoice_number.' (#'.$gi->purchase_invoice_no.')',"amount"=>$gi->total_amount];
                                }
                            }
                              $getar = \app\models\TblVendorAccountPayable::find()
                                      ->where('status != 0')
                                      ->andWhere("DATE(crt_time) BETWEEN '".$from_date."' AND '".$to_date."'")
                                      ->andWhere('fk_vendor_invoice_id in (select id from tbl_vendor_invoice where fk_vendor_id = '.$customer.' and status != 0 and fk_location_id = '.$bill_company.')')
                            ...
                            if(isset($getar) && count($getar) > 0){
                                foreach($getar as $gi){
                                    ...
                                    if(isset($getinvoice) && $getinvoice->id != ""){
                                        $invoice_no = " (Invoice #".$getinvoice->vendor_invoice_number.", Vendor Invoice #".$getinvoice->purchase_invoice_no.")";
                                    }else{
                                        $invoice_no = " (Invoice #.)";
                                    }
                                        $ar_arr[] = ['date'=>date('Y-m-d',strtotime($gi->crt_time)),"type"=>"Amount Paid","number"=>$gi->id.$invoice_no,"amount"=>$gi->amount_received];
                            ...
                            if(count($mergedArray) > 0){
                                ...
                                }else if($mergedArray[0]['type'] == "Amount Paid"){
                                    $starting_balance = -$mergedArray[0]['amount'];
            ...
            <table id="outstanding-bal" class="display table table-hover table-bordered" data-page-length="20">
                <thead>
                    ...
                    <tr>
                        ...
                        <td class="filterhead">Payment Made</td>
                ...
                <tbody>
                    <?php
                        ...
                        if(isset($from_date) && isset($to_date) && isset($customer)){
                            ...
                            if($bill_company != 0){
                              $getallinvoices = \app\models\TblVendorInvoice::find()
                                              ->where('status != 0')
                            ...
                            if(isset($getallinvoices) && count($getallinvoices) > 0){
                                foreach($getallinvoices as $gi){
                                    ...
                                    $a_r = \app\models\TblVendorAccountPayable::find()->where(['fk_vendor_invoice_id'=>$gi->id])->andWhere('status != 0')->all();
                                    $a_r_amount = 0;
                                    foreach($a_r as $ar){
                                    ...
                                    if(!($gi->total_amount == $a_r_amount)){
                                        ...
                                        <td>'.$gi->vendor_invoice_number.'</td>
                                        <td>'.$gi->purchase_invoice_no.'</td>
        ...
        <hr />
        <h3 class="mb-3">Vendor Information</h3>
        <div class="table-responsive">
            <table id="customer-list" class="display table table-hover table-bordered">
                ...
                <tbody>
                    <?php
                        if(isset($customer)){
                            $getcustomer = TblVendor::find()->where(['id'=>$customer])->andWhere('status != 0')->one();
