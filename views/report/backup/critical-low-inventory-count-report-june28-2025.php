...
$this->title = 'Critically Low Inventory Count Report (Below 80% of the reorder level)';
...
<div class="card">
  ...
  <div class="card-body border-top">
    <form name="frm_reservation" id="" action="<?= Yii::$app->urlManager->createUrl(['report/critical-low-inventory-count-report']) ?>" method="post" autocomplete="off">
            ...
            foreach($products as $p){
              ...
              if (strpos($locations, ',') !== false) {
                  ...
                  foreach ($locationIds as $locationId) {
                      // echo "Location ID: " . trim($locationId) . "<br>";
                      //---get bin location -----
                      $getbin = TblBinLocations::find()->where(['status'=>1,'id'=>trim($locationId)])->one();
                      if(isset($getbin) && $getbin->id != ""){
                        //------now get area, row, bay, level and position -----

                        if($bin_locs == ""){
                            $bin_locs .= $getbin->area.' - '.$getbin->row.' - '.$getbin->bay.' - '.$getbin->level.' - '.$getbin->position;
                        }else{
                            $bin_locs .= '<hr />'.$getbin->area.' - '.$getbin->row.' - '.$getbin->bay.' - '.$getbin->level.' - '.$getbin->position;
                        }

                      }
                  }//----for loop ended ----------
              } else {
                  // Single location, no commas
                  // echo "Single Location ID: " . $locations . "<br>";
                  //---get bin location -----
                  $getbin = TblBinLocations::find()->where(['status'=>1,'id'=>$locations])->one();
                  if(isset($getbin) && $getbin->id != ""){
                    //------now get area, row, bay, level and position -----
              ...
              echo '<tr>
              <td>'.$p['product_name'].' <a class="unformat text-sm" href="'.Url::to(['product/update','id'=>$p['product_id']]).'" target="_blank"><i class="fas fa-external-link-alt" data-bs-toggle="tooltip" data-bs-placement="right" title="Open in New Tab"></i></a></td>
              <td>'.$p['description'].'</td>
