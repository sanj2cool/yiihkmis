...
<div class="card">
  <div class="card-header bg-light">
    ...
    <form name="frm_reservation" id="" action="<?= Yii::$app->urlManager->createUrl(['report/customer-aging-report']) ?>" method="post" autocomplete="off">
        ...
        <div class="col-md-3">
          <label for="">Bill Company<span class="text-danger">*</span></label>
          <select class="form-select" name="bill_company" required>
            <option value="0" <?php if(isset($bill_company)){ if($bill_company == 0){ echo 'selected="selected"';}}else{ echo 'selected="selected"';}?>>All</option>
            <?php
              ...
              if(isset($ownership_companies) && count($ownership_companies) > 0){
                foreach($ownership_companies as $oc){
                  if(isset($bill_company) && $bill_company == $oc->id){
                      echo '<option value="'.$oc->id.'" selected="selected">'.$oc->location_name.'</option>';
                  }else{
      ...
      <table id="example" class="display table table-hover table-bordered" data-page-length="20">
          ...
          <tbody>
            ...
            if (!empty($bill_company)) {
                if($bill_company != 0){
                  $query->andWhere(['i.fk_bill_from_id' => $bill_company]);
                }
            }
