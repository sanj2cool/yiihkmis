        ...
        <div class="table-responsive">
          <table id="customer-list" class="display table table-hover table-bordered" data-page-length="20">
            ...
            <tbody>
              ...
              $total_payment = 0;
              $i = 1;
              if(isset($from_date) && isset($to_date) && isset($vendor)){

                if($vendor == 0){
                  //------all the terms and crt by
                  $model = TblVendorAccountPayable::find()
                  ->where('status != 0')
                  ->andWhere("DATE(ar_date) BETWEEN '".$from_date."' AND '".$to_date."'")
                  // ->andWhere('fk_vendor_id in (select fk_vendor_id from tbl_vendor_invoice where status = 1 and fk_location_id = '.$location_selected.')')
                ...
                foreach($model as $m){
                    ...
                    // $getvendor = TblVendor::find()->where(['id'=>$m->fk_vendor_id,'status'=>1])->one();
