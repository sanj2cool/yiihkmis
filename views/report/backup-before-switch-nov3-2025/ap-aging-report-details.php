          ...
          <tbody>
            ...
            $query = (new Query())
                ->select([
                    ...
                    'i.due_date',
                    'i.total_amount',
                    'COALESCE(SUM(ir.amount_received), 0) + COALESCE(SUM(ir_credit.amount_applied), 0) AS total_amount_received',
                    'i.total_amount - COALESCE(SUM(ir.amount_received), 0) - COALESCE(SUM(ir_credit.amount_applied), 0) AS outstanding_amount',
                    'SUM(CASE
                            WHEN DATEDIFF(CURDATE(), i.due_date) BETWEEN 0 AND 15 THEN i.total_amount - COALESCE(ir.amount_received, 0) - COALESCE(ir_credit.amount_applied, 0)
                            ELSE 0
                        END) AS current',
                    'SUM(CASE
                            WHEN DATEDIFF(CURDATE(), i.due_date) BETWEEN 16 AND 30 THEN i.total_amount - COALESCE(ir.amount_received, 0) - COALESCE(ir_credit.amount_applied, 0)
                            ELSE 0
                        END) AS days_16_30',
                    'SUM(CASE
                            WHEN DATEDIFF(CURDATE(), i.due_date) BETWEEN 31 AND 60 THEN i.total_amount - COALESCE(ir.amount_received, 0) - COALESCE(ir_credit.amount_applied, 0)
                            ELSE 0
                        END) AS days_31_60',
                    'SUM(CASE
                            WHEN DATEDIFF(CURDATE(), i.due_date) BETWEEN 61 AND 90 THEN i.total_amount - COALESCE(ir.amount_received, 0) - COALESCE(ir_credit.amount_applied, 0)
                            ELSE 0
                        END) AS days_61_90',
                    'SUM(CASE
                            WHEN DATEDIFF(CURDATE(), i.due_date) > 90 THEN i.total_amount - COALESCE(ir.amount_received, 0) - COALESCE(ir_credit.amount_applied, 0)
                            ELSE 0
                        END) AS over_90_days'
                ])
    ...
    footerCallback: function (row, data, start, end, display) {
      ...
      var formatNumber = function (num) {
        return num.toLocaleString(undefined, {
          minimumFractionDigits: ".Yii::$app->params['decimalPlaces'].",
          maximumFractionDigits: ".Yii::$app->params['decimalPlaces']." // Allows up to 2 decimal places but not forced
        });
      };
      // Update footer
