...
if($model->isNewRecord){
  ...
  $get = TblVendorInvoice::find()->where(['status'=>1])->orderBy(['id'=>SORT_DESC])->one();
  if(isset($get) && $get->id != ""){
    $invoice_no = intval($get->vendor_invoice_number)+1;
    $new_number = sprintf('%04d', $invoice_no);
  }else{
    $invoice_no = 1001;
    $new_number = sprintf('%04d', $invoice_no);
  }
  $model->vendor_invoice_number = $new_number;
  $model->hst = 9;
}

if(!$model->isNewRecord){
  $allporders = TblPurchaseOrder::find()->where(['status'=>1])->andWhere(['fk_location_id'=>$fk_location_id,'fk_vendor_id'=>$model->fk_vendor_id])->orderBy(['crt_by'=>SORT_DESC])->all();
  $poarr = ArrayHelper::map($allporders,'id','po_number');
}else{
  $poarr = [];
}
...
<div class="tbl-vendor-invoice-form">
  <div id="msg">
  ...
  <div class="card shadow rounded mt-4">
    ...
    <div class="card-body p-3">
      <div class="row">
        <div class="col-lg-2">
          <?= $form->field($model, 'vendor_invoice_number')->textInput(['maxlength' => true,'readonly'=>true,'class'=>'form-control bg-light']) ?>
        </div>
        <div class="col-lg-2">

          <?= $form->field($model, 'purchase_invoice_no')->textInput(['maxlength' => true]) ?>
        </div>
        <div class="col-lg-4">
        ...
        <div class="col-lg-2">
          <?= $form->field($model, 'fk_purchase_order_id')->dropDownList($poarr,['prompt'=>'Select']) ?>
        </div>
        <div class="col-lg-2">
      ...
      <div class="row">
        ...
        <div class="col-lg-3">
          <?= $form->field($model, 'invoice_date')->textInput(['class'=>'form-control']) ?>
        </div>
        <div class="col-lg-3">
          <?= $form->field($model, 'due_date')->textInput(['class'=>'form-control']) ?>
        </div>

        <div class="col-lg-3">
          <label for="">Upload Invoice</label>
          <input type="file" name="vendor_invoice" accept="image/*,application/pdf" class="form-control">
          <?php
          if(!$model->isNewRecord && $model->invoice_file != ""){
            ...
            if ($fileExtension === 'pdf') {
              echo '<a class="btn btn-success btn-sm mt-2" href="javascript:void(0);" style="cursor:pointer;" onclick="handleFileAction(\'' . $uploaded_file . '\')"><i class="fas fa-file-invoice"></i> View Uploaded Invoice</a>';
            }else{
              echo '<input type="hidden" id="img_image_1" value="https://hkmis.ca/web/vendor-invoices/'.$model->invoice_file.'">';
              echo '<a class="btn btn-success btn-sm img_open_popup mt-2" style="cursor:pointer;"  id="image_1">
      ...
      if(!$model->isNewRecord){
        ...
        if(isset($getitems) && count($getitems) > 0){
          ...
          foreach($getitems as $gi){
            //---if product receving entry does not exists for the existing item give the option to create one ---
            $show_receving = '';
            $receiving_count = \app\models\TblProductReceiving::find()->where(['fk_vendor_invoice_item_id'=>$gi->id,'status'=>1])->count();
            if($receiving_count == 0){
                $show_receving = '<div class="row g-1 mt-1 align-items-center receiving-row">

                  <div class="col-lg-3">
                    <div class="form-check">
                      <input class="form-check-input create-receiving"
                             type="checkbox"
                             name="product_receiving_'.$i.'"
                             value="1"
                             checked>
                  ...
                  <div class="col-lg-3">
                    <input type="text"
                           name="bin_location_'.$i.'"
                           class="form-control form-control-sm"
                           placeholder="Bin Location (A-01-2)">
                  </div>

                  <div class="col-lg-6">
                    <input type="text"
                           name="receiving_remarks_'.$i.'"
                           class="form-control form-control-sm"
            ...
            if($i == 0){
              ...
              <div class="row g-1">
                <div class="col-lg-3" id="product_div_0">
                  <label>Product</label>
                  <input type="hidden" name="item_id[]" value="<?=$gi->id?>" id="item_id_<?=$i?>">
              ...
              ?>
              <div class='product-row mb-2 row_extra mt-2' data-index='<?=$i?>'>
          ...
          <div class="row g-1">
            <div class="col-lg-3" id="product_div_0">
              ...
              <div class="mt-1">
                  <input type="checkbox" name="product_receiving_0" value="1" checked/> Create Receiving
              </div>
        ...
        <div class="product-row mb-2" data-index="0">
          ...
          <!-- ROW 2 : RECEIVING INFO -->
  <div class="row g-1 mt-1 align-items-center receiving-row">

    <div class="col-lg-3">
      <div class="form-check">
        <input class="form-check-input create-receiving"
               type="checkbox"
               name="product_receiving_0"
               value="1"
               checked>
        <label class="form-check-label small fw-semibold">
          Create Receiving
  ...
  if(!$model->isNewRecord){
    ...
    if($model->discount_type == 1 && $model->discount_value != ""){
      ...
      $dis_value = number_format($dis_value,2,'.','');
    }else{
      $dis_value = 0;
    }
    ...
    $new_subtotal = $subtotal-$dis_value;
    $tax_on = $new_subtotal;
  }else{
    $subtotal = 0.00;
    $hst_amount = 0.00;
      ...
      <div class="row">
        ...
        <div class="col-lg-4 text-end">
          <div class="row g-3" style="font-weight:600;">
            ...
            <input type="hidden" id="subtotal" name="TblVendorInvoice[subtotal]" value="<?=$subtotal?>">
            <input type="hidden" id="hst_amount" name="TblVendorInvoice[hst_amount]" value="<?=$hst_amount?>">
            <input type="hidden" id="total" name="TblVendorInvoice[total_amount]" value="<?=$total_amount?>">
          ...
          <div class="row g-3">
            ...
            <div class="col-lg-3 mt-3" id="discount_calc">
              -$<?=$dis_value?>
            </div>
            <div class="text-danger" id="discount_err">
...
if(isset($getproducts) && count($getproducts) > 0){
  foreach($getproducts as $gp){
    ...
    $productinternal_sku = htmlspecialchars(str_replace(array("\r", "\n"), ' ', $gp->website_description), ENT_QUOTES); // Escape description and replace line breaks
    // $product_str .= "<option value='".$gp->id."'>".$gp->name." [".$gp->internal_sku."]</option>";
    $product_str .= "<option value='" . $productId . "'>" . $productName . "[".$productinternal_sku."]</option>";
  }
}
$register_js_var = <<<JS
...
$(document).on("click",".btn_add_project",function(e){
  ...
  let str = "<div class='product-row mb-2 row_extra mt-2' data-index='"+cc+"'><hr class='text-primary'/><div class='row g-1'><div class='col-lg-3' id='product_div_"+cc+"'><select class='form-control product_select' name='product[]' id='product_"+cc+"'><option value=''>Select</option>'.$product_str.'</select></div><div class='col-lg-4'><input type='text' name='desc[]' id='desc_"+cc+"' placeholder='Description' class='form-control' required='required'></div><div class='col-lg-1'><input type='text' name='qty[]' id='qty_"+cc+"' placeholder='0' class='form-control qty' required='required' onkeyup='checkNum(this);'></div><div class='col-lg-2'><input type='text' name='rate[]' id='rate_"+cc+"' placeholder='0' class='form-control floatNumberField rate' required='required' onkeyup='checkDec(this);'></div><div class='col-lg-1'><input type='text' name='amount[]' id='amount_"+cc+"' placeholder='0' class='form-control amount bg-light' tabindex='-1' readonly='readonly'></div><div class='col-lg-1'><button class='btn btn-primary btn_add_project btn-sm'><i class='fa fa-plus'></i></button>&nbsp;&nbsp;<button class='btn btn-primary btn_remove_project btn-sm'><i class='fa fa-minus'></i></button></div></div><div class='row g-1 mt-1 align-items-center receiving-row'><div class='col-lg-3'><div class='form-check'><input class='form-check-input create-receiving' type='checkbox' name='product_receiving_"+cc+"' value='1' checked><label class='form-check-label small fw-semibold'>Create Receiving</label></div></div><div class='col-lg-3'><input type='text' name='bin_location_"+cc+"' class='form-control form-control-sm' placeholder='Bin Location (A-01-2)'></div><div class='col-lg-6'><input type='text' name='receiving_remarks_"+cc+"' class='form-control form-control-sm' placeholder='Receiving remarks (optional)'></div></div></div>";
...
$(document).on("click",".btn_remove_project",function(e){
  e.preventDefault();
  $(this).parent().parent().parent(".row_extra").remove();
  calculatetotalwithtaxes();
});
//--------------terms logic as per customer selection starts----------------
var due_date_picker = $("#tblvendorinvoice-due_date").flatpickr({
  disableMobile: "true",
  dateFormat: "Y-m-d"
});
var invoice_date_picker = $("#tblvendorinvoice-invoice_date").flatpickr({
  disableMobile: "true",
  dateFormat: "Y-m-d"
});
var dealChoices;

// Initialize Choices for Deal ID
function initDealChoices() {
    var dealSelect = document.getElementById('tblvendorinvoice-fk_purchase_order_id');

    if (dealChoices) {
        dealChoices.destroy();
    }

    dealChoices = new Choices(dealSelect, {
...
initDealChoices();
$("#tblvendorinvoice-fk_vendor_id").change(function(){
  var fk_vendor_id = $("#tblvendorinvoice-fk_vendor_id").val();
  console.log("fk_vendor_id id::::"+fk_vendor_id);
  $.post("index.php?r=vendor-invoice/getvendor",{fk_vendor_id:fk_vendor_id},function(r){
    console.log(r);

    var terms = r.terms;
    if(terms !== 0){
      $("#tblvendorinvoice-fk_terms_id").val(terms).change();
      setDueDateTerms();
    }//---------client has terms selected-------
    let response_arr = r.purchase;
    if(response_arr){
      dealChoices.clearChoices();
      dealChoices.setChoices(response_arr, 'value', 'label', true);
    }

  });
});
function setDueDateTerms(){
  var term_val = $("#tblvendorinvoice-fk_terms_id").val();
  var invoice_date = $("#tblvendorinvoice-invoice_date").val();
...
$("#tblvendorinvoice-fk_terms_id").change(function(){
  setDueDateTerms();
});
$("#tblvendorinvoice-invoice_date").change(function(){
  setDueDateTerms();
});
...
$("#tblvendorinvoice-hst").change(function(){
  calculatetotalwithtaxes();
});
$("#tblvendorinvoice-discount_type").change(function(){
  calculatetotalwithtaxes();
});
$("#tblvendorinvoice-discount_value").keyup(function(){
  calculatetotalwithtaxes();
});
...
function calculatetotalwithtaxes(){
  ...
  var discount_value = $("#tblvendorinvoice-discount_value").val();
  var discount_type = $("#tblvendorinvoice-discount_type").val();
  var disc_percentage = 0;
  let dis_value = 0;
  let discount_calculated = 0;
  if(discount_type == 1 && discount_value != 0.00){
    //-----its simple value just subtract from the subtotal -------
    dis_value = discount_value;
  }else if(discount_type == 2 && discount_value != 0.00){
    //------its %age --------
    let discount_percentage = parseFloat(sub_total)*(discount_value/100);
    dis_value = discount_percentage;
    dis_value = dis_value.toFixed(2);
  }
  console.log("discount value::::::::"+dis_value);
  $("#discount_calc").html("-$"+dis_value);
  let new_subtotal = parseFloat(sub_total)-parseFloat(dis_value);
  new_subtotal = new_subtotal.toFixed(2);
  console.log("new sub total::::"+new_subtotal);
  //--------this is the old code ----------
  // let hst_value = $("#tblvendorinvoice-hst").val();
  // if(hst_value !== "" && $.isNumeric(hst_value)){
    //     total_hst = sub_total*(hst_value/100);
    // }
    //
    // total_hst = total_hst.toFixed(2);
    //--------this is the old code ----------
    //--------new code for hst ----------
    var selectedOption = $("#tblvendorinvoice-hst").find("option:selected");
    var selectedOptionId = $("#tblvendorinvoice-hst").val();
    ...
    if(taxPercentage !== "" && $.isNumeric(taxPercentage)){
      total_hst = new_subtotal*(taxPercentage/100);
    }

    total_hst = total_hst.toFixed(2);

    let total_amount = parseFloat(new_subtotal)+parseFloat(total_hst);
    document.getElementById("subtotal_calc").innerHTML = "$"+sub_total;
    $("#hst_val").html("Tax @ "+taxPercentage+"% on $"+new_subtotal);
...
JS;
$this->registerJs($register_js_var);
  ?>
