<?php

use yii\helpers\Html;
use yii\widgets\ActiveForm;
use app\models\TblProduct;
use app\models\TblPrintedBarcodes;
use yii\helpers\ArrayHelper;
use app\models\TblBinLocations;
use app\models\TblProductReceiving;
$session = Yii::$app->session;
$user_company = $session['userCompany'];
//-----get products --------
$allbins = TblProduct::find()->where(['status'=>1])->andWhere('id not in (select fk_product_id from tbl_printed_barcodes WHERE status = 1)')->all();

$binarr = ArrayHelper::map($allbins, 'id', function($model) {
  $bin_location = getLatestLocation($model->id);
  return strtoupper($model->name).' | '.$model->internal_sku.' | '.strtoupper($model->description).' | '.strtoupper($bin_location);
});
//---function to get current bin location of the product -----
//---add fk_location_id as per logged in location -----

function getLatestLocation($product_id){
  $session = Yii::$app->session;
  $user_company = $session['userCompany'];
    $bin_location = "";
    $getprec = TblProductReceiving::find()->where(['status'=>1,'fk_product_id'=>$product_id,'fk_location_id'=>$user_company])->orderBy(['crt_time'=>SORT_DESC])->one();
    if(isset($getprec) && $getprec->id != ""){
      $bin_location = $getprec->bin_location;
    }
    return $bin_location;

}
// print_r($binarr);
$this->title = "Print Barcodes";

use yii\helpers\Url;
use app\models\TblMenu;
//=======get the mennu items with parent_id = 18 =======
//====order by priority =======
$getmenuitems = TblMenu::find()->where(['parent_id'=>18,'status'=>1])->orderBy(['priority'=>SORT_ASC])->all();
//======current menu id is 19=======
if(isset($getmenuitems) && count($getmenuitems) > 0){
  echo '<nav aria-label="breadcrumb mb-2">
  <ol class="breadcrumb mb-0">';
  foreach($getmenuitems as $gm){
    if($gm->id == 70){
      echo ' <li class="breadcrumb-item active" aria-current="page">'.$gm->title.'</li>';
    }else{
      echo '<li class="breadcrumb-item"><a href="'.Url::to([$gm->url]).'">'.$gm->title.'</a></li>';
    }
  }
  echo '  </ol>
</nav>';
}//======if isset ended ========
?>
    <style>
    @page {
            size: 21.6cm 27.9cm;
            margin: 0;
        }

        body {
            margin: 0;
            padding: 0;
        }

        .label-sheet {
            width: 21.6cm;
            height: 27.9cm;
            margin: 0;
            padding-top: 1.2cm;
            padding-left: 0.4cm;
            box-sizing: border-box;
            display: flex;
            flex-wrap: wrap;
            align-content: flex-start;
        }

        .label {
            width: 10.1cm;
            height: 2.55cm;
            border: 1px solid grey;
            border-radius: 0.5rem;
            /* display: flex; */
            align-items: center;
            justify-content: center;
            text-align: center;
            color: #000;
            box-sizing: border-box;
            margin: 0;
            font-size:35px;
            font-weight:900;
        }

        .label:nth-child(2n) {
            margin-left: 0.4cm; /* Space between columns */
        }

        .no-print {
            margin: 20px;
            text-align: center;
        }

        @media print {
            body * {
                visibility: hidden;
            }
            .label-sheet, .label-sheet * {
                visibility: visible;
            }
            .label-sheet {
                position: absolute;
                left: 0;
                top: 0;
                width: 21.6cm;
                height: 27.9cm;
            }
            .no-print {
                display: none;
            }
        }
       </style>
       <div id="msg">


           <div class="alert alert-outline-success d-flex align-items-center hidden" role="alert" id="success_msg">
             <span class="fas fa-check-circle text-success fs-5 me-3"></span>
             <p class="mb-0 flex-1">Printed barcodes removed successfully.</p>

             <button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button>
           </div>

           <div class="alert alert-outline-danger d-flex align-items-center hidden" role="alert" id="error_msg">
             <span class="fas fa-times-circle text-danger fs-5 me-3"></span>
             <p class="mb-0 flex-1">Something went wrong. Please try again.</p>
             <button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button>
           </div>


       </div>
       <div class="card shadow rounded mt-2">
      <div class="card-body p-3">
          <div class="row">
              <div class="col-lg-6">
                  <h3>Generate Printable Barcodes</h3>
              </div>
              <div class="col-lg-6 text-end">
                    <button type="button" class="btn btn-danger btn-sm hidden" id="btn_remove">Remove the Printed Barcodes from the List</button>

                  <button id="printButton" class="btn btn-primary btn-sm">Print Barcodes</button>
              </div>
          </div>
      </div>
  </div>

  <div class="card shadow rounded mt-2 mb-2">
      <div class="card-body p-3">
          <div class="container">
              <form id="labelForm" class="mb-4 row">
                  <div class="mb-3 col-lg-9">
                      <label for="labelSelect" class="form-label">Select 20 options:</label>
                      <select id="labelSelect" class="form-select" multiple required>
                          <?php
                          foreach($binarr as $key => $value){
                              echo '<option value="'.$key.'">'.$value.'</option>';
                          }
                          ?>
                      </select>
                      <div class="mt-3">
                          <h5 class="fw-bold">Selected Product Quantities</h5>

                          <div id="qtyContainer" class="row g-3"></div>

                          <div class="mt-3">
                              <span id="totalCounter" class="fw-bold fs-5">Total Labels: 0 / 20</span>
                              <button type="button" id="resetQty" class="btn btn-secondary btn-sm ms-3">Reset Quantities</button>
                          </div>
                      </div>


                  </div>
                  <div class="col-lg-3 mt-4">
                    <button type="submit" class="btn btn-primary btn-sm">Create Labels</button>
                    <a class="btn btn-danger btn-sm" href="<?=Url::to(['site/print-barcodes'])?>">Reset</a>
                  </div>

              </form>

              <div class="row">
                <div class="col-12">
                  <p class="text-danger text-center fw-bold">
                    Please Select Paper Size - A4, Margin Settings - None and Scale - Default when printing the labels.
                  </p>
                </div>
              </div>

          </div>
      </div>
  </div>
  <div id="print_content_container">
      <div id="labelSheetContainer" class="label-sheet"></div>
  </div>


  <style>
  .qty-box {
    border: 1px solid #ddd;
    border-radius: 6px;
    padding: 10px;
    background: #fafafa;
}

.qty-box label {
    font-size: 14px;
}

.counter-over {
    color: red !important;
}

  </style>
<?php
$jsR = <<<JS
var maxLabels = 20;

var labelSelect = new Choices('#labelSelect', {
    removeItemButton: true,
    maxItemCount: 20,
    searchEnabled: true,
    shouldSort: false
});


// -------------------------
// LIVE TOTAL COUNTER LOGIC
// -------------------------
function updateTotalCounter() {
    var qtyInputs = document.querySelectorAll('.barcode-qty');
    var total = 0;

    for (var i = 0; i < qtyInputs.length; i++) {
        total += parseInt(qtyInputs[i].value, 10);
    }

    var counter = document.getElementById('totalCounter');
    counter.innerHTML = "Total Labels: " + total + " / " + maxLabels;

    if (total > maxLabels) {
        counter.classList.add('counter-over');
    } else {
        counter.classList.remove('counter-over');
    }

    return total;
}


// -------------------------
// ADD QUANTITY INPUT
// -------------------------
labelSelect.passedElement.element.addEventListener('addItem', function (event) {
    var itemId = event.detail.value;

    // Prevent duplicates
    if (document.getElementById('qty_' + itemId)) return;

    var qtyCol = document.createElement('div');
    qtyCol.className = "col-md-6";
    qtyCol.id = "qty_" + itemId;

    qtyCol.innerHTML =
        '<div class="qty-box">' +
            '<label class="fw-bold d-block">' + event.detail.label + '</label>' +
            '<input type="number" min="1" value="1" class="form-control form-control-sm barcode-qty" ' +
            'data-product-id="' + itemId + '">' +
        '</div>';

    document.getElementById('qtyContainer').appendChild(qtyCol);

    updateTotalCounter();
});


// -------------------------
// REMOVE QUANTITY INPUT
// -------------------------
labelSelect.passedElement.element.addEventListener('removeItem', function (event) {
    var itemId = event.detail.value;
    var elem = document.getElementById('qty_' + itemId);

    if (elem) elem.parentNode.removeChild(elem);

    updateTotalCounter();
});


// -------------------------
// QUANTITY CHANGE HANDLER
// -------------------------
document.addEventListener('input', function (event) {
    if (event.target.classList.contains('barcode-qty')) {
        updateTotalCounter();
    }
});


// -------------------------
// RESET BUTTON
// -------------------------
document.getElementById('resetQty').addEventListener('click', function () {
    var qtyInputs = document.querySelectorAll('.barcode-qty');

    for (var i = 0; i < qtyInputs.length; i++) {
        qtyInputs[i].value = 1;
    }

    updateTotalCounter();
});


// -------------------------
// FORM SUBMIT = VALIDATION + LABEL GENERATION
// -------------------------
document.getElementById('labelForm').addEventListener('submit', function (event) {
    event.preventDefault();

    var totalQty = updateTotalCounter();

    if (totalQty > maxLabels) {
        alert("❌ You selected " + totalQty + " labels. Only 20 labels fit on one sheet.");
        return false;
    }

    if (totalQty < 1) {
        alert("Please select at least one product.");
        return false;
    }

    document.getElementById('btn_remove').classList.remove('hidden');

    var labelSheetContainer = document.getElementById('labelSheetContainer');
    labelSheetContainer.innerHTML = "";

    var selectedOptions = labelSelect.getValue();

    for (var s = 0; s < selectedOptions.length; s++) {

        var option = selectedOptions[s];
        var qtyInput = document.querySelector('input[data-product-id="' + option.value + '"]');
        var qty = qtyInput ? parseInt(qtyInput.value, 10) : 1;

        for (var q = 0; q < qty; q++) {

            var arr = option.label.split('|');

            var partname = arr[0].trim();
            var internal_sku = arr[1].trim();
            var partdesc = arr[2].trim();
            //---get substr by 36 characters ----
            let desclength = partdesc.length;
            if(desclength > 36){
              partdesc = partdesc.substring(0, 36)+'...';
            }
            var partlocation = arr[3].trim();

            var labelDiv = document.createElement('div');
            labelDiv.className = 'label';

            var barcodeContainer = document.createElement('div');
            barcodeContainer.style.textAlign = 'center';

            var barcodeImage = document.createElement('img');
            barcodeImage.src = 'product-barcodes/' + internal_sku + '.png';
            barcodeImage.style.display = 'inline';

            var textDiv = document.createElement('div');
            textDiv.style.fontFamily = 'monospace';
            textDiv.style.fontSize = '12px';
            textDiv.textContent = 'PART: ' + partname + ', BIN: ' + partlocation;

            var textDiv2 = document.createElement('div');
            textDiv2.style.fontFamily = 'monospace';
            textDiv2.style.fontSize = '12px';
            textDiv2.textContent = 'DESC: ' + partdesc;

            barcodeContainer.appendChild(barcodeImage);
            barcodeContainer.appendChild(textDiv);
            barcodeContainer.appendChild(textDiv2);

            labelDiv.appendChild(barcodeContainer);

            labelSheetContainer.appendChild(labelDiv);
        }
    }
});


// --------------------------
// PRINT
// --------------------------
document.getElementById('printButton').addEventListener('click', function () {
    window.print();
});

JS;
$this->registerJs($jsR);
  $this->registerJs("


      // Function to remove selected choices and reinitialize
    function removeSelectedChoices() {
      // Remove selected choices
         labelSelect.removeActiveItems();

         // Make an AJAX call to get refreshed data from the server
         fetch('index.php?r=site/get-barcodes')  // Replace with your actual endpoint
             .then(response => response.json())
             .then(data => {
                 // Assuming `data` is an array of objects like [{ value: '1', label: 'Option 1' }, { value: '2', label: 'Option 2' }]


                 console.log(data);
                 let obj = JSON.parse(data);
                 // Clear the current choices
                 labelSelect.clearChoices();

                 // Add new choices from the PHP response
                 labelSelect.setChoices(obj, 'value', 'label', false);

             })
             .catch(error => {
                 console.error('Error fetching data:', error);
             });
    }

    $('#btn_remove').click(function(e){
      e.preventDefault();
       if (confirm('Are you sure you want to remove the printed labels?') == true) {
         let selected_labels = $('#labelSelect').val();
         console.log(selected_labels);
         $.post('index.php?r=site/remove-barcodes-from-list',{labels:selected_labels},function(r){
           // console.log(r);
           if(r == 'success'){
             // alert('Printed labels removed successfully');
             removeSelectedChoices();
             $('#btn_remove').addClass('hidden');
             $('#success_msg').removeClass('hidden');
           }else{
             // alert('Something went wrong. Please try again.');
             $('#error_msg').removeClass('hidden');
           }
         });
       }

    });
  ");
 ?>
