...
<div class="tbl-user-form">
    ...
    <!-- Menu end -->
    <div class="card shadow rounded mt-4">
      <div class="card-header p-3 bg-soft-danger">
        <div class="row">
          <div class="col-lg-6">
            <h4>Report Access</h4>
          </div>
          <div class="col-lg-6 text-end">
            <input type="checkbox" id="select_all_report">&nbsp;&nbsp;<strong>Select All</strong>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-lg-12 table-responsive">
            ...
            <table class="table table-bordered">
              ...
              <tbody>
                <?php
                    $getcountreport = TblReport::find()->where(['!=','id',1])->andWhere(['status'=>1])->orderBy(['priority' => SORT_ASC])->all();
                    $getallreports = TblReport::find()->where(['!=','id',1])->andWhere(['parent_id'=>1])->andWhere(['status'=>1])->orderBy(['priority' => SORT_ASC])->all();
                    $r = 0;
                    echo '<input type="hidden" name="reportitems" value="'.count($getcountreport).'">';
                    foreach($getallreports as $gr){
                      //check if report has children
                      $getrchild = TblReport::find()->where(['parent_id'=>$gr->id])->andWhere(['status'=>1])->orderBy(['priority'=>SORT_ASC])->all();
                      if(isset($getrchild) && count($getrchild) > 0){
                        if(!$model->isNewRecord){
                          $getraccess = TblReportAccess::find()->where(['fk_user_id'=>$model->id])->andWhere(['fk_report_id'=>$gr->id])->andWhere(['status'=>1])->one();
                          echo '<tr class="table-info">
                          <td><input type="hidden" name="reportitem_'.$r.'" value="'.$gr->id.'">'.$gr->title.'</td>
                          <td>';
                          if(isset($getraccess) && $getraccess->view_crud == 1){
                              echo '<input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="1" class="report_view_hidden">
                              <input type="checkbox" class="reportviewparent" id="'.$r.'" checked="checked">
                              </td>';
                          }else{
                            echo '<input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="0" class="report_view_hidden">
                            <input type="checkbox" class="reportviewparent" id="'.$r.'">
                            </td>';
                          }
                          echo '</tr>';
                        }else{
                          ...
                          echo '<tr class="table-info">
                          <td><input type="hidden" name="reportitem_'.$r.'" value="'.$gr->id.'">'.$gr->title.'</td>
                          <td><input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="1" class="report_view_hidden">
                          <input type="checkbox" class="reportviewparent" id="'.$r.'" checked="checked">
                          </td>
                          </tr>';
                        }
                        ...
                        $rclassvar = $r;
                        foreach($getrchild as $gc){
                          $r++;
                          $rcheck_child = TblReport::find()->where(['parent_id'=>$gc->id])->andWhere(['status'=>1])->orderBy(['priority'=>SORT_ASC])->all();
                          if(isset($rcheck_child) && count($rcheck_child) > 0){
                            //has third level children
                            if(!$model->isNewRecord){
                              $getrchildaccess = TblReportAccess::find()->where(['fk_user_id'=>$model->id])->andWhere(['fk_report_id'=>$gc->id])->andWhere(['status'=>1])->one();
                              echo '<tr class="table-warning">
                              <td><input type="hidden" name="reportitem_'.$r.'" value="'.$gc->id.'">'.$gc->title.'</td>';
                              if(isset($getrchildaccess) && $getrchildaccess->view_crud == 1){
                                echo '<td><input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="'.$getrchildaccess->view_crud.'"  class="report_view_hidden report_view_hide_'.$rclassvar.'">
                                <input type="checkbox" checked="checked" name="rview_'.$r.'" id="rview_'.$r.'" class="rview viewreport_'.$rclassvar.'"></td>';
                              }else{
                                echo '<td><input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="0"  class="report_view_hidden report_view_hide_'.$rclassvar.'">
                                <input type="checkbox" name="rview_'.$r.'" id="rview_'.$r.'" class="rview viewreport_'.$rclassvar.'"></td>';
                              }
                              echo '</tr>';
                            }else{
                              echo '<tr class="table-warning">
                              <td><input type="hidden" name="reportitem_'.$r.'" value="'.$gc->id.'">'.$gc->title.'</td>
                              <td><input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="1"  class="report_view_hidden report_view_hide_'.$rclassvar.'">
                              <input type="checkbox" checked="checked" name="rview_'.$r.'" id="rview_'.$r.'" class="rview viewreport_'.$rclassvar.'"></td>
                              </tr>';
                            }

                            foreach($rcheck_child as $cc){
                              $r++;
                              if(!$model->isNewRecord){
                                echo '<tr>
                                <td style="padding-left:20px"><input type="hidden" name="reportitem_'.$r.'" value="'.$cc->id.'">'.$cc->title.'</td>';
                                $getthirdchildaccess = TblReportAccess::find()->where(['fk_user_id'=>$model->id])->andWhere(['fk_report_id'=>$cc->id])->andWhere(['status'=>1])->one();
                                if(isset($getthirdchildaccess) && $getthirdchildaccess->view_crud == 1){
                                  echo '<td><input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="'.$getthirdchildaccess->view_crud.'"  class="report_view_hidden report_view_hide_'.$rclassvar.'">
                                  <input type="checkbox" checked="checked" name="rview_'.$r.'" id="rview_'.$r.'" class="rview viewreport_'.$rclassvar.'"></td>';
                                }else{
                                  echo '<td><input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="0"  class="report_view_hidden report_view_hide_'.$rclassvar.'">
                                  <input type="checkbox" name="rview_'.$r.'" id="rview_'.$r.'" class="rview viewreport_'.$rclassvar.'"></td>';
                            ...
                            }//thid level children for loop ended
                          }else{
                            //no third level children
                            if(!$model->isNewRecord){
                                $getrchildaccess = TblReportAccess::find()->where(['fk_user_id'=>$model->id])->andWhere(['fk_report_id'=>$gc->id])->andWhere(['status'=>1])->one();
                                ...
                                <td><input type="hidden" name="reportitem_'.$r.'" value="'.$gc->id.'">'.$gc->title.'</td>';
                                if(isset($getrchildaccess) && $getrchildaccess->view_crud == 1){
                                  echo '<td><input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="1"  class="report_view_hidden report_view_hide_'.$rclassvar.'">
                                  <input type="checkbox" checked="checked" name="rview_'.$r.'" id="rview_'.$r.'" class="rview viewreport_'.$rclassvar.'"></td>';
                                }else{
                                  echo '<td><input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="0"  class="report_view_hidden report_view_hide_'.$rclassvar.'">
                                  <input type="checkbox" name="rview_'.$r.'" id="rview_'.$r.'" class="rview viewreport_'.$rclassvar.'"></td>';
                            ...
                            }//new record else ended
                          }//no third level children else ended
                        }//for loop of second level children ended
                      }else{
                        //no children single report item
                        if(!$model->isNewRecord){
                          $getrmenuaccess = TblReportAccess::find()->where(['fk_user_id'=>$model->id])->andWhere(['fk_report_id'=>$gr->id])->one();
                          echo '<tr class="table-warning">
                          <td><input type="hidden" name="reportitem_'.$r.'" value="'.$gr->id.'">'.$gr->title.'</td>
                          <td>';
                          if(isset($getrmenuaccess) && $getrmenuaccess->view_crud == 1){
                              echo '<input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="1"   class="report_view_hidden">
                              <input type="checkbox" checked="checked" name="rview_'.$r.'" id="rview_'.$r.'" class="rview">';
                          }else{
                              echo '<input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="0"   class="report_view_hidden">
                              <input type="checkbox" name="rview_'.$r.'" id="rview_'.$r.'" class="rview">';
                          }
                          echo '</td>
                          </tr>';
                        }else{
                          ...
                          <td><input type="hidden" name="reportitem_'.$r.'" value="'.$gr->id.'">'.$gr->title.'</td>
                          <td><input type="hidden" name="reportview_'.$r.'" id="reportview_'.$r.'" value="1"   class="report_view_hidden">
                          <input type="checkbox" checked="checked" name="rview_'.$r.'" id="rview_'.$r.'" class="rview"></td>
                          </tr>';
                        }
