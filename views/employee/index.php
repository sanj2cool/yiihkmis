...
/** @var app\models\TblEmployeeSearch $searchModel */
    ...
    <div class="row">
      ...
      <div class="col-lg-6 text-end">
        ...
        if(isset($menuaccess_c) && $menuaccess_c->create_crud == 1){
         ?>
        <?= Html::a('Create Employee', ['create'], ['class' => 'btn btn-primary btn-sm']) ?>
        <?php
          }
         ?>
      </div>
...
<div class="tbl-employee-index table-responsive">
        ...
        'columns' => [
            ...
            [
              ...
              'urlCreator' => function ($action, $model, $key, $index) {
                if ($action === 'edit') {
                  $url ='index.php?r=employee/update&id='.$model->id;
                  return $url;
                }
                if ($action === 'delete') {
                  $url ='index.php?r=employee/delete&id='.$model->id;
                  return $url;
                }
              }
            ],
            ...
            [
              ...
              'value' => function($dataProvider){
                $getrole = TblRoleMaster::find()->where(['id'=>$dataProvider->fk_role_id,'status'=>1])->one();
                if(isset($getrole) && $getrole->title != ""){
                  return $getrole->title;
                }else{
                  return "(not set)";
                }
              },
              'filter' => Html::activeDropDownList($searchModel, 'fk_role_id', $rolearr,['class'=>'form-select','prompt' => 'Select']),
            ],
            //'ip',
            // 'status',
            [
              ...
              'filter' => Html::activeDropDownList($searchModel, 'status', $status,['class'=>'form-select','prompt' => 'Select']),
            ]
            //'crt_by',
            //'mod_by',
            //'crt_time',
            ...
            // [
            //     'class' => ActionColumn::className(),
            //     'urlCreator' => function ($action, TblEmployee $model, $key, $index, $column) {
