...
$allporders = TblPurchaseOrder::find()->where(['status'=>1])->andWhere(['fk_location_id'=>$fk_location_id])->orderBy(['crt_by'=>SORT_DESC])->all();
...
<div class="tbl-vendor-invoice-form">
  ...
  <div class="card shadow rounded mt-4">
    <div class="card-body p-3">
      <div class="row">
        <div class="col-lg-2">
          <?= $form->field($model, 'vendor_invoice_number')->textInput(['maxlength' => true,'readonly'=>true]) ?>
        </div>
        <div class="col-lg-4">
        ...
        <div class="col-lg-4">
          <?= $form->field($model, 'fk_purchase_order_id')->dropDownList($poarr,['prompt'=>'Select','data-choices'=>'data-choices','data-options'=>'{"removeItemButton":true,"placeholder":true}']) ?>
        </div>
        <div class="col-lg-2">
    ...
    <div class="card-body p-3">
      <div class="row">
        ...
        <div class="col-lg-2">
          ...
          if(!$model->isNewRecord){
            $check = TblVendorAccountPayable::find()->where(['fk_vendor_invoice_id'=>$model->id])->one();
            if(isset($check) && $check->id != ""){
              $amount_received = $check->amount_received;
            }else{
              $amount_received = 0;
            }
            // echo $amount_received."::::".$total;
            if($amount_received == $model->total_amount){
              ?>
              <input type="checkbox" name="amount_paid_old" value="Yes" checked="checked" style="display:none;">
              <input type="checkbox" name="amount_paid" value="Yes" checked="checked">
              <?php
            }else{
              ?>
              <input type="checkbox" name="amount_paid_old" value="Yes" style="display:none;">
