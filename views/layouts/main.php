          ...
          <div class="row w-50">
            <div class="col-3 pe-0 px-0">
              <div class=" d-none d-lg-block d-sm-block w-100">
                <select class="form-select form-select-sm" name="select-search-opt" id="select-search-opt">
                  <?php
...
<!-- ===============================================-->
  ...
  function performSearch() {
    var search = $("#search_by_part").val();
    var search_type = $("#select-search-opt").val();

    if (search.length < 2) { // require at least 2 chars
    ...
    $.ajax({
      url: "index.php?r=site/search-by-part-new",
      type: "POST",
      dataType: "json",
      data: {
        search: search,
      ...
      success: function (obj) {
        var return_count = obj.return_count || 0;
        var return_str   = obj.return_str || "";
        $("#search_result_count").html(return_count);
        $("#searchbar_content_div").html(return_str);
      },
      error: function () {
        $("#search_result_count").html(0);
        $("#searchbar_content_div").html("<div class='px-3 py-2 text-danger'>Error loading results</div>");
      }
    });
  }

  $("#search_by_part").on("keyup", function () {
    clearTimeout(searchTimer);
    searchTimer = setTimeout(performSearch, 250);
  });

  // Run search when user changes search type as well
  $("#select-search-opt").on("change", function () {
    performSearch();
  });
});
