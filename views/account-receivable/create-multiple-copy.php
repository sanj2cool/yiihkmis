...
<div class="tbl-account-receivable-create">
  ...
  <form method="post" action="<?=Url::to(['account-receivable/create-multiple-copy'])?>" enctype="multipart/form-data" id="form_multiple">
    ...
    <div class="card shadow rounded mt-4">
      ...
      <div class="card-body p-3">
        <div class="row">
          ...
          <div class="col-lg-3 mb-2">
            <label for="">Receiving Company<span class="text-danger">*</span><sup><span class="badge bg-danger">New</span></sup></label>
            <select class="form-select" name="bill_from_company" id="bill_from_company" required>
              <option value="">Select</option>
              <?php
              $allbillfrom = TblOwnershipCompany::find()->where(['status'=>1])->orderBy(['company_name'=>SORT_ASC])->all();
              if(isset($allbillfrom) && count($allbillfrom) > 0){
                foreach($allbillfrom as $c){
                  if($c->id == 1){
                    echo '<option value="'.$c->id.'" selected="selected">'.$c->location_name.'</option>';
                  }else{
                    echo '<option value="'.$c->id.'">'.$c->location_name.'</option>';
                  }
                }//---for loop ended -----
              }//-----if isset ended -----
              ?>
            </select>
            <div id="billfrom_error"class="text-danger"></div>
          </div>
...
function getclientinvoices(){
  ...
  if(error == 0){
    $.post("index.php?r=account-receivable%2Fget-pending-invoice-copy",{client_id:client_id,bill_from_company:bill_from_company},function(r){
      console.log(r);
