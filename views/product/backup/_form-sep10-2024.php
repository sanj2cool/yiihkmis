      ...
      <div class="tab-pane fade show active" id="home" role="tabpanel">
        ...
        <div class="card shadow rounded mt-2">
          <div class="card-header p-4 border-bottom border-300 bg-soft">
           <div class="row g-3 justify-content-between align-items-center">
             <div class="col-12 col-md">
               <h4 class="text-900 mb-0" data-anchor="data-anchor" id="pricing-details">Pricing Details <sup><badge class="badge bg-danger">New</badge></sup></h4>
             </div>
           </div>
         </div>
            <div class="card-body p-3">
                      ...
                      if(!$model->isNewRecord){
                        ...
                        if(isset($getproductsreceived) && count($getproductsreceived) > 0){
                          foreach($getproductsreceived as $rs){
                            ...
                            $getbin = TblBin::find()->where(['id'=>$rs->fk_bin_id,'status'=>1])->one();
                            if(isset($getbin) && $getbin->id != ""){
                              //-------get area ------
                              $getarea = TblBinArea::find()->where(['status'=>1,'id'=>$getbin->fk_area_id])->one();
                              ...
                              //---------get row ------
                              $getrow = TblBinRow::find()->where(['status'=>1,'id'=>$getbin->fk_row_id])->one();
                              ...
                              //---------get bay -------
                              $getbay = TblBinBay::find()->where(['status'=>1,'id'=>$getbin->fk_bay_id])->one();
                              ...
                              //---------get level -------
                              $getlevel = TblBinLevel::find()->where(['status'=>1,'id'=>$getbin->fk_level_id])->one();
                              ...
                              //---------get position -------
                              $getposition = TblBinPosition::find()->where(['status'=>1,'id'=>$getbin->fk_position_id])->one();
                              if(isset($getposition) && $getposition->title != ""){
                                $position = $getposition->title;
                              }else{
                                $position = "";
                              }
                              $bin = $area.' - '.$row.' - '.$bay.' - '.$level.' - '.$position;
                            }else{
                              $bin = "(not set)";
                            }
                            echo '<tr>
