...
$getreceived_recent = TblProductReceiving::find()->where(['status'=>1,'fk_product_id'=>$model->id,'fk_location_id'=>$user_company])->orderBy(['id'=>SORT_DESC])->one();
...
//---get categories string as well -----
$getselectedcats = \app\models\TblProductAssignedCategory::find()->where(['fk_product_id'=>$model->id,'status'=>1])->all();
$cats_str = "";
