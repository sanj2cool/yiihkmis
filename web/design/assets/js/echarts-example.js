(function (factory) {
  ...
  if ($chartEl) {
    ...
    fetch('/web/index.php?r=site/get-chart-data') // Replace with your actual URL
      .then(response => response.json())
      .then(data => {
        const { months, invoiceAmounts, arAmounts } = data;
        ...
        const getDefaultOptions = () => ({
          ...
          legend: {
            top: 40,
            data: ['Invoice Amounts', 'Account Receivables'],
            textStyle: {
              color: getColor('tertiary-color')
            }
          },
          ...
          yAxis: [
            {
              type: 'value',
              min: Math.min(...invoiceAmounts, ...arAmounts),
              max: Math.max(...invoiceAmounts, ...arAmounts) * 1.1,
              interval: 30000, // Dynamically calculate interval
              // min: Math.min(...invoiceAmounts, ...arAmounts), // Calculate minimum value
              // max: Math.max(...invoiceAmounts, ...arAmounts) + 10, // Calculate maximum with a buffer
              // interval: calculateInterval(Math.min(...invoiceAmounts, ...arAmounts), Math.max(...invoiceAmounts, ...arAmounts)), // Dynamically calculate interval
              axisLabel: {
                color: getColor('quaternary-color'),
                formatter: '{value}'
              },
        ...
        // Calculate dynamic interval function (optional, adjust as needed)
        function calculateInterval(min, max) {
          const range = max - min;
          const steps = Math.ceil(range / 25); // Adjust divisor for fewer intervals
      ...
      .catch(error => {
        ...
        // Handle error here, display a message or use default values
      });
  }
  ...
  //       legend: {
  //         top: 40,
  //         data: ['Invoices', 'Account Receivables'],
  //         textStyle: {
  //           color: getColor('tertiary-color')
  //         }
  //       },
