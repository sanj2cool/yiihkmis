...
use app\models\TblProductCategorySearch;
...
use yii\helpers\Url;
/**
 * ProductCategoryController implements the CRUD actions for TblProductCategory model.
 */
class ProductCategoryController extends Controller
{
    ...
    /**
     * Lists all TblProductCategory models.
     *
     * @return string
     */
    ...
    /**
     * Displays a single TblProductCategory model.
     * @param int $id ID
     * @return string
    ...
    /**
     * Creates a new TblProductCategory model.
              ...
              if(isset($_FILES["cat_img"]) && $_FILES["cat_img"]["name"] != ""){
                ...
                if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"
                  ...
                  if(move_uploaded_file($_FILES["cat_img"]["tmp_name"], $target_file)){
                    $modelPic = TblProductCategory::find()->where(['id'=>$model->id])->one();
    ...
    /**
     * Updates an existing TblProductCategory model.
    ...
    /**
     * Deletes an existing TblProductCategory model.
    ...
    public function actionDelete($id)
    {
      ...
      if(!isset($session['username'])){
        ...
        // $this->findModel($id)->delete();
          ...
          if($check == ""){
            $model = TblProductCategory::find()->where(['id'=>$id])->one();
            $model->status = 0;
            if($model->save()){
    ...
    private function CheckBeforeDelete($id){
      ...
      $check = TblProduct::find()
              ->where('status != 0')
              ->andWhere('id in (select fk_product_id from tbl_product_assigned_category where status = 1 and fk_category_id = '.$id.')')
    ...
    /**
     * Finds the TblProductCategory model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return TblProductCategory the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TblProductCategory::findOne(['id' => $id])) !== null) {
            return $model;
        }
