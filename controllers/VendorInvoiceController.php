...
class VendorInvoiceController extends BaseController
{
                  ...
                  if(isset($_FILES["doc_file"]["name"][$i]) && $_FILES["doc_file"]["name"][$i] != ""){
                    ...
                    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"
                      ...
                      if(move_uploaded_file($_FILES["doc_file"]["tmp_name"][$i], $target_file)){
                        ...
                        //====$file_name======
                        $modelDoc = new \app\models\TblVendorInvoiceFile();
                        $modelDoc->fk_vendor_invoice_id = $model->id;
                        $modelDoc->invoice_file = $file_name;
                        $modelDoc->save();
                      }
    ...
    public function actionDelete($id)
    {
        ...
        if($model->save()){
          ...
          \app\models\TblVendorInvoiceFile::updateAll($updatearr,'fk_vendor_invoice_id='.$id);
    ...
    public function actionDelpics(){
      $del_id = $_POST['del_id'];
      $model = \app\models\TblVendorInvoiceFile::find()->where(['id'=>$del_id])->one();
      $model->status = 0;
      if($model->save()){
        return "yes";
      }else{
	...
	public function actionCreateFromPo($id)
{
    $session = Yii::$app->session;
    if (!isset($session['username'])) {
        return $this->redirect(['site/login']);
    ...
    $po = \app\models\TblPurchaseOrder::find()
    ...
    try {
        ...
        // ✅ LOCK + GET LAST NUMBER (inside transaction)
        $lastInvoice = Yii::$app->db->createCommand("
        ...
        $lastNumber = isset($lastInvoice['vendor_invoice_number']) && is_numeric($lastInvoice['vendor_invoice_number'])
            ? (int)$lastInvoice['vendor_invoice_number']
            : 0;

        $nextNumber = $lastNumber > 0 ? $lastNumber + 1 : 1000;

        // ✅ Create Invoice
        $invoice = new TblVendorInvoice();
        $invoice->purchase_invoice_no   = '0000';
        $invoice->vendor_invoice_number = (string)$nextNumber;

        $invoice->fk_vendor_id   = $po->fk_vendor_id;
        $invoice->fk_location_id = $po->fk_location_id;
        ...
        $invoice->hst_amount   = $po->hst_amount;
        $invoice->total_amount = $po->total_amount;

        $invoice->po_id = $po->id;

        if (!$invoice->save()) {
            throw new \Exception(json_encode($invoice->errors));
        }

        // ✅ Copy Items
        $poItems = \app\models\TblPurchaseOrderItem::find()
            ->where(['fk_purchase_order_id' => $po->id, 'status' => 1])
            ->all();

        foreach ($poItems as $item) {
            $invItem = new TblVendorInvoiceItem();
            $invItem->fk_vendor_invoice_id = $invoice->id;
            $invItem->fk_product_id        = $item->fk_product_id;
            $invItem->description          = $item->description;
            $invItem->quantity             = $item->quantity;
            $invItem->unit_price           = $item->unit_price;
            $invItem->total_price          = $item->total_price;

            if (!$invItem->save()) {
                throw new \Exception(json_encode($invItem->errors));
            }
        }
        ...
        return $this->redirect(['update', 'id' => $invoice->id]);
