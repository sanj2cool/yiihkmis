...
use app\models\TblProductCategoryCopySearch;
...
use yii\helpers\Url;
/**
 * ProductCategoryController implements the CRUD actions for TblProductCategoryCopy model.
 */
class ProductCategoryCopyController extends Controller
{
    /**
     * @inheritDoc
     */
    ...
    /**
     * Lists all TblProductCategoryCopy models.
     *
     * @return string
     */
    public function actionIndex()
    {
      ...
      if(!isset($session['username'])){
        ...
        $searchModel = new TblProductCategoryCopySearch();
        $params = Yii::$app->request->queryParams;
        // $dataProvider = $searchModel->search($this->request->queryParams);
        // If category_id is passed, ensure it's set in the search model
        if (isset($params['parent_id'])) {
            $searchModel->parent_id = $params['parent_id'];
        }
    ...
    /**
     * Displays a single TblProductCategoryCopy model.
     * @param int $id ID
     * @return string
    ...
    /**
     * Creates a new TblProductCategoryCopy model.
    ...
    public function actionCreate()
    {
      ...
      if(!isset($session['username'])){
        ...
        $model = new TblProductCategoryCopy();

        if ($this->request->isPost) {
            if ($model->load($this->request->post()) && $model->save()) {
              //-----CHECK FOR FILE UPLOADED ----
              $msg_doc = "";
              $target_dir = "../web/product-cat-images/";
              if(isset($_FILES["cat_img"]) && $_FILES["cat_img"]["name"] != ""){
                ...
                if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"
                  ...
                  if(move_uploaded_file($_FILES["cat_img"]["tmp_name"], $target_file)){
                    $modelPic = TblProductCategoryCopy::find()->where(['id'=>$model->id])->one();
    ...
    /**
     * Updates an existing TblProductCategoryCopy model.
    ...
    /**
     * Deletes an existing TblProductCategoryCopy model.
    ...
    public function actionDelete($id)
    {
      ...
      if(!isset($session['username'])){
        ...
        // $this->findModel($id)->delete();
          ...
          if($check == ""){
            $model = TblProductCategoryCopy::find()->where(['id'=>$id])->one();
            $model->status = 0;
            if($model->save()){
              ...
              <thead>
                <tr class="table-danger" align="center">
                  <th colspan="6" align="center">
                    There are product(s) related to this category. Please remove the category from product(s) and try again.
                  </th>
                </tr>
                <tr>
                <th>Sr. No.</th>
    ...
    private function CheckBeforeDelete($id){
      ...
      if(isset($check) && count($check) > 0){
        ...
        foreach($check as $c){
          ...
          $getcat = TblProductCategoryCopy::find()->where(['id'=>$c->fk_category_id,'status'=>1])->one();
    ...
    /**
     * Finds the TblProductCategoryCopy model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return TblProductCategoryCopy the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TblProductCategoryCopy::findOne(['id' => $id])) !== null) {
            return $model;
        }
