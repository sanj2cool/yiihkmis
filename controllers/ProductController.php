...
class ProductController extends Controller
{
  ...
  public function actionDelpics(){
    ...
    if (isset($data['name'])) {
      ...
      $normalizedPart = strtolower(str_replace([' ', '-', '.', '_'], '', $part_name));
      ...
      $checkProductCross = TblProductCrossRef::find()
          ...
          ->andWhere([
              ...
              new \yii\db\Expression("
                  LOWER(REPLACE(REPLACE(REPLACE(REPLACE(cross_ref, ' ', ''), '-', ''), '.', ''), '_', ''))
                  =
                  :normalizedPart
              "),
              ['like',
          ...
          ->addParams([':normalizedPart' => $normalizedPart])
          ->groupBy(['fk_product_id'])
          ->all();
    ...
    public function actionSearch(){
      ...
      if(isset($getproduct) && $getproduct->id != ""){
        if($getproduct->status == 1){
          ...
          $getselectedcats = \app\models\TblProductAssignedCategory::find()->where(['fk_product_id'=>$getproduct->id,'status'=>1])->all();
          $cats_str = "";
