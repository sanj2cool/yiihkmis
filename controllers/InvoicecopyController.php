...
class InvoicecopyController extends Controller
{
    ...
    public function actionSendInvoice($id){
      ...
      $filePath = "/var/www/html/web/invoices/".$filename; // Path to your file
      ...
      // if (filter_var(trim($vendor_email), FILTER_VALIDATE_EMAIL)) {
        ...
        if (empty($send_arr['to'])) {
          Yii::$app->getSession()->setFlash('error', 'No valid Email-id not found. Please enter valid email-id and try again.');
          echo 'No valid Email-id not found. Please enter valid email-id and try again.';
        }else{
          echo 'Email id found';
            // $curl = curl_init();
            //
    ...
    public function createpdf($id){
      ...
      if(isset($model) && $model->id != ""){
        ...
        $filename = "/var/www/html/web/invoices/".$file;
        if($balance_due == 0){
          //---paid ===
          $pdf = new Pdf([
            'mode' => Pdf::MODE_CORE,
            ...
            'methods' => [
              'SetHeader'=>[],
              'SetFooter'=>['{PAGENO}'],
              'SetWatermarkImage' => ['/var/www/html/web/images/paid-invoice.png'],
            ]
          ]);
